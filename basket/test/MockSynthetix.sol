...
contract MockSynthetix is ISynthetix {
    ...
    function exchange(
        ...
        uint256 outputAmount =
            _sourceAmount.mul(sourcePrice).div(destinationPrice);

        getOrSetToken(_sourceCurrencyKey).burn(
            _sourceAmount.sub(subtractSourceAmount),
            msg.sender
        );
        getOrSetToken(_destinationCurrencyKey).mint(
            outputAmount.sub(subtractOutputAmount),
            msg.sender
        );
    }
    ...
    function getOrSetToken(bytes32 _currencyKey) public returns (MockToken) {
        if (address(keyToToken[_currencyKey]) == address(0)) {
            keyToToken[_currencyKey] = new MockToken(
                string(abi.encode(_currencyKey)),
                string(abi.encode(_currencyKey))
            );
