...
contract RSISynthetixManager {
    ...
    constructor(
        ...
        require(
            _rsiBottom < _rsiTop,
            "RSI bottom should be bigger than RSI top"
        );
    ...
    function long() internal {
        ...
        require(
            address(currentToken) == assetShort,
            "Can only long when short"
        );
        ...
        // sanity checks
        require(
            currentToken.balanceOf(address(basket)) == 0,
            "Current token balance should be zero"
        );
        require(
            IERC20(assetLong).balanceOf(address(basket)) >= 10**6,
            "Amount too small"
        );
    ...
    function getCurrentToken() public view returns (address) {
        ...
        require(
            tokens.length == 1,
            "RSI Pie can only have 1 asset at the time"
        );
        return tokens[0];
    }
    ...
    function readRound(uint256 _round)
        public
        view
