...
contract FeeRewardForwarder is Governable {
    ...
    address public constant yfi = address(
    ...
    constructor(address _storage, address _uniswapRouterV2)
        public
        Governable(_storage)
    {
    ...
    function poolNotifyFixedTarget(address _token, uint256 _amount) external {
        ...
        if (_token == targetToken) {
            ...
            if (uniswapRoutes[_token][targetToken].length > 1) {
                ...
                uint256 convertedRewardAmount = IERC20(targetToken).balanceOf(
                    address(this)
                );
                IERC20(targetToken).safeTransfer(
                    profitSharingPool,
