...
contract SingleNativeTokenExit {
    ...
    function exitEth(ExitTokenStruct calldata _exitTokenStruct) external {
        ...
        IBasketFacet(address(inputBasket)).exitPool(_exitTokenStruct.inputAmount, _exitTokenStruct.referral);

        address[] memory tokens =
            IBasketFacet(address(inputBasket)).getTokens();
        ...
        uint256 intermediateTokenBalance =
            INTERMEDIATE_TOKEN.balanceOf(address(this));
        require(intermediateTokenBalance >= _exitTokenStruct.minAmount, "Insufficient output");
